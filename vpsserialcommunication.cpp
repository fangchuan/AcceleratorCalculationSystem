...
SerialCommunication::SerialCommunication() : itk::Object(),
...
int SerialCommunication::Receive(std::string& answer, unsigned int numberOfBytes, const char *eol)
{
  if (numberOfBytes == 0)
    return OK;
...
int SerialCommunication::Send(const std::string& input, bool block)
{
...
int SerialCommunication::ApplyConfiguration()
{
...
void SerialCommunication::SendBreak(unsigned int ms)
{
#ifdef WIN32
  if (m_ComPortHandle == INVALID_HANDLE_VALUE)
    return;
...
void SerialCommunication::ClearSendBuffer()
{
#ifdef WIN32
  if ( m_ComPortHandle != INVALID_HANDLE_VALUE )
