...
NDIErrorCode NDIProtocol::INIT()
{
	return GenericCommand("INIT");
}


NDIErrorCode NDIProtocol::DSTART()
{
	return GenericCommand("DSTART");
}


NDIErrorCode NDIProtocol::DSTOP()
{
	return GenericCommand("DSTOP");
}


NDIErrorCode NDIProtocol::IRINIT()
{
	return GenericCommand("IRINIT");
}


NDIErrorCode NDIProtocol::IRCHK(bool* IRdetected)
{
...
NDIErrorCode NDIProtocol::PHSR(PHSRQueryType queryType, std::string* portHandles)
{
...
NDIErrorCode NDIProtocol::PHRQ(std::string* portHandle)
{
	...
	else  // No error, expect port handle as a 2 character hexadecimal value
	{
		*portHandle = reply;                      // assign the port handle to the return string
		...
		if (expectedCRC == readCRC)               // if the read CRC is correct, return okay
			...
			/*returnValue = NDICRCERROR;*/
			returnValue = NDIOKAY; // 修改，修正其他电脑出错
	}
	/* cleanup and return */
	m_TrackingDevice->ClearReceiveBuffer();     // flush the buffer to remove the remaining carriage return or unknown/unexpected reply
	return returnValue;
...
NDIErrorCode NDIProtocol::PVWR(std::string* portHandle, const unsigned char* sromData, unsigned int sromDataLength)
{
	NDIErrorCode returnValue = NDIUNKNOWNERROR; // return code for this function. Will be set according to reply from trackingsystem
...
NDIErrorCode NDIProtocol::PINIT(std::string* portHandle)
{
	NDIErrorCode returnValue = NDIUNKNOWNERROR; // return code for this function. Will be set according to reply from trackingsystem
...
NDIErrorCode NDIProtocol::PENA(std::string* portHandle, TrackingPriority prio)
{
	std::string param;
	if (portHandle != NULL)
...
NDIErrorCode NDIProtocol::PHINF(std::string portHandle, std::string* portInfo)
{
	std::string command;
	if (m_UseCRC) command = "PHINF:" + portHandle;
	else command = "PHINF " + portHandle;
	NDIErrorCode returnValue = m_TrackingDevice->Send(&command, m_UseCRC);
...
NDIErrorCode NDIProtocol::PDIS(std::string* portHandle)
{
	return this->GenericCommand("PDIS", portHandle);
}


NDIErrorCode NDIProtocol::PHF(std::string* portHandle)
{
	return this->GenericCommand("PHF", portHandle);
}


NDIErrorCode NDIProtocol::IRATE(IlluminationActivationRate rate)
{
	std::string param;
	switch (rate)
	{
...
NDIErrorCode NDIProtocol::BEEP(unsigned char count)
{
	NDIErrorCode returnValue = NDIUNKNOWNERROR; // return code for this function. Will be set according to reply from trackingsystem
...
NDIErrorCode NDIProtocol::TSTART(bool resetFrameCounter)
{
	std::string param = "80";
...
NDIErrorCode NDIProtocol::TSTOP()
{
	return this->GenericCommand("TSTOP");
}


NDIErrorCode NDIProtocol::PSOUT(std::string portHandle, std::string state)
{
...
NDIErrorCode NDIProtocol::TX(bool trackIndividualMarkers, MarkerPointContainerType* markerPositions)
{
	NDIErrorCode returnValue = NDIUNKNOWNERROR; // return code for this function. Will be set according to reply from tracking device
	...
	else                                        // transformation data is returned
	{
		...
		for (unsigned int i = 0; i < numberOfHandles; i++)    // for each handle
		{
			...
			else  // transformation data
			{
				...
				Quaternion orientation(localQuat[1], localQuat[2], localQuat[3], localQuat[0]);

				//If the rotation mode is vnlTransposed we have to transpose the quaternion
				if (m_TrackingDevice->GetRotationMode() == NDITrackingDevice::RotationTransposed)
				{
					orientation[0] *= -1; //qx
					orientation[1] *= -1; //qy
					orientation[2] *= -1; //qz
...
NDIErrorCode NDIProtocol::TX1000(MarkerPointContainerType* markerPositions)
{
...
NDIErrorCode NDIProtocol::VER(TrackingDeviceType& t)
{
	if (m_TrackingDevice == NULL)
		return TRACKINGDEVICENOTSET;
...
NDIErrorCode NDIProtocol::POS3D(MarkerPointContainerType* markerPositions)
{
	NDIErrorCode returnValue = NDIUNKNOWNERROR; // return code for this function. Will be set according to reply from trackingsystem
	...
	NDIErrorCode receivevalue = m_TrackingDevice->Receive(&reply, 3);       // read first 3 characters of reply (error or number of markers returned)
...
unsigned int NDIProtocol::ByteToNbBitsOn(char& c) const
{
	if(c == '0')
		return 0;
...
NDIErrorCode NDIProtocol::ParseOkayError()
{
	NDIErrorCode returnValue  = NDIUNKNOWNERROR;
...
NDIErrorCode NDIProtocol::GetErrorCode(const std::string* input)
{
...
NDIErrorCode NDIProtocol::APIREV(std::string* revision)
{
	if (m_TrackingDevice == NULL)
		return TRACKINGDEVICENOTSET;
...
NDIErrorCode NDIProtocol::SFLIST(std::string* info)
{
	if (m_TrackingDevice == NULL)
		return TRACKINGDEVICENOTSET;
		...
		if (numberOfVolumes>0)
		{
			...
			for (unsigned int i = 0; i<numberOfVolumes; i++)
			{
				...
				if (currentVolume.compare(0,1,DeviceDataPolarisOldModel.HardwareCode)==0)
					std::cout <<"Standard volume supported \n";
				else if (currentVolume.compare(0,3,DeviceDataPolarisSpectra.HardwareCode)==0)
					std::cout <<"Spectra pyramid volume supported \n";
				else if (currentVolume.compare(0,3,DeviceDataSpectraExtendedPyramid.HardwareCode)==0)
					std::cout <<"Spectra extended pyramid volume supported \n";
				else if (currentVolume.compare(0,1,DeviceDataPolarisVicra.HardwareCode)==0)
					std::cout <<"Vicra volume supported \n";
				else if (currentVolume.compare(0,1,DeviceDataAuroraPlanarCube.HardwareCode)==0)
					std::cout <<"Cube volume supported \n";
				else if (currentVolume.compare(0,1,DeviceDataAuroraPlanarDome.HardwareCode)==0)
					std::cout <<"Dome volume supported \n";
				else
...
NDIErrorCode NDIProtocol::VSEL(TrackingDeviceData deviceData)
{
	if (m_TrackingDevice == NULL)
		return TRACKINGDEVICENOTSET;
	...
	NDITrackingDevice::NDITrackingVolumeContainerType volumes;
	NDITrackingDevice::TrackingVolumeDimensionType volumesDimensions;
	...
	NDITrackingDevice::NDITrackingVolumeContainerType::iterator it = volumes.begin();
	while (it != volumes.end())
	{
		if ((*it) == deviceData.Model)
		{
			std::cout << deviceData.Model << " selected.";
			break;
		}
		it++, index++;
	}
